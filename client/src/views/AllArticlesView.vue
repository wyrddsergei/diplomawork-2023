...
export default {
  ...
  setup() {
    ...
    const articles = ref([])
    const route = useRoute() // Move useRoute() inside the setup() function

    const fetchPosts = () => {
      const searchQuery = route.query.search || ''
      const url = `http://localhost:4000/api/posts?page=${currentPage.value}&limit=${limit}&search=${searchQuery}`
    ...
    const goToPage = (page) => {
      if (page !== currentPage.value) {
        ...
        // const url = `/articles?page=${page}&search=${searchQuery}`
